...
Abbildung 1 und 2 lassen keine deutliche Bevorzugung einzelner Schläge erkennen, außer vielleicht, den Komplex Rotes Kreuz / Mitterbachl und dem Klemmtaferl. Bei ersterem wäre die naheliegendste These zur Erklärung der höheren Individuendichte, das Vorhandensein von Gewässern.
Da die Schläge nur ein mäßiges Mittel zur Beantwortung dieser Frage sind, kann es gut sein, dass vorhandene Effekte mit dieser Methode nicht aufgezeigt werden können.
...
Desweiteren können andere Effekte Standorteffekte überlagern; ein möglicher Einflussfaktor ist die Anzahl der Kästen pro Schlag, welcher als nächstes untersucht wird.
...
Es können also maximal geringe geographische Präferenzen beobachtet werden, dafür aber eine positiv-lineare Beziehung zwischen Kastendichte und Individuendichte. 
Dieser Zusammenhang bedeutet, dass sich die Population durch das Aufhängen neuer Kästen stärken lässt, aber auch, dass sie sinken wird, wenn die Kastdichte aufgrund des hohen Alters vieler Kästen abnimmt.

# Wie steht es um Quantität und Qualität der künstlichen Quartiere?

```{r Tab.7, results="asis"}
# Subsets für jedes Jahr
anno10 <- subset(gesamt, koa == 10)
anno11 <- subset(gesamt, koa == 11)
anno12 <- subset(gesamt, koa == 12)
anno13 <- subset(gesamt, koa == 13)
anno14 <- subset(gesamt, koa == 14)
anno15 <- subset(gesamt, koa == 15)

# Anzahl der gefundenen Kästen pro Jahr
schlag10 <- summary(na.exclude(anno10$schlag))
schlag11 <- summary(na.exclude(anno11$schlag))
schlag12 <- summary(na.exclude(anno12$schlag))
schlag13 <- summary(na.exclude(anno13$schlag))
schlag14 <- summary(na.exclude(anno14$schlag))
schlag15 <- summary(na.exclude(anno15$schlag))

Schlag <- data.frame(g.g, schlag10, schlag11, schlag12, schlag13, schlag14, schlag15)
names(Schlag) <- c("gesamt", "2010", "2011", "2012", "2013", "2014", "2015")

Table7 <- xtable(
		 Schlag,
		 caption="Tabelle 7: Anzahl der Kästen pro Schlag und Jahr",
		 digits=0
		 )
